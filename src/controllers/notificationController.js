...
const createNotification = async (req, res) => {
    try {
        const { type, recipientType, title, message, relatedId, relatedModel, actionUrl } = req.body;
...
const contactAdmin = async (req, res) => {
    try {
        ...
        res.status(201).json({
            success: true,
            message: 'Your request has been sent to admin. You will be contacted soon.',
            data: notification,
        });
    } catch (error) {
        res.status(500).json({
